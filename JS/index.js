...
const cardHeandle = async (categoryId) => {
    ...
    data?.data.forEach(card  => {
        ...
        const div = document.createElement ('div');
        div.classList = `card w-72 max-h-[350px] bg-base-100  ml-3`
        div.innerHTML = `
                ...
                <div class="card-body">
                    ...
                    <div class="flex">
                        <p class="ml-12 text-gray-400 font-normal text-sm">${card?.authors[0]?.profile_name} </p>
                        <img src="${card?.authors[0]?.card?.authors[0]? `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        </svg>` : ''}" alt="Verification Icon">
                    </div>
                    <p class="text-gray-400 ml-10"> ${card?.others?.views} views <p>
                    <p class="text-gray-400 ml-10"> ${card?.others?.posted_date} <p>
                </div>`
